    ...
    async function loadNarrativePublications() {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/publications/section/NARRATIVA`
        );
  ...
  return (
    <section className="publications-container theory-section" style={{ backgroundImage: `linear-gradient(rgba(10,6,4,0.55), rgba(10,6,4,0.55)), url(${banner80})`, backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed" }}>
      <header className="narrative-header">
        <h1 className="narrative-page-title">
          <span className="narrative-page-main">L I Z &nbsp; L U I S &nbsp; Q U I R O G A</span>
          <span className="narrative-page-sub">Narrativa</span>
        </h1>
        <div className="narrative-page-rule" />
      ...
      {publications.length === 0 ? (
        <p className="text-neutral-500 text-sm tracking-wide">
          No hay publicaciones de narrativa todavía.
        </p>
      ) : (
        <>
        {filtered.length === 0 && (
